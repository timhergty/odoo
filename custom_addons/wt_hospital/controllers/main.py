...
class Hospital(http.Controller):
    ...
    # @http.route('/hospital/patients/', website=True, auth='user')
    @http.route('/hospital/patients/', website=True, auth='public')
    def hospital_patient(self, **kw):
        # return "Hospital Patients Page"
        patients = request.env['hospital.patient'].sudo().search([])
        return request.render("wt_hospital.patients_page", {
            'patients': patients
        })
    ...
    def hospital_doctor(self, **kw):
        # return "Hospital Doctors Page"
        doctors = request.env['hospital.doctor'].sudo().search([])
        return request.render("wt_hospital.doctors_page", {
            'doctors': doctors
        })
